...
if [ ! -f "${SCRIPT_DIR}/cloudformation/parameters/${ENVIRONMENT}.env" ]; then
    echo "Error: cloudformation/parameters/${ENVIRONMENT}.env not found"
    exit 1
fi
...
docker build -f "${SCRIPT_DIR}/document-storage-service/Dockerfile" -t "document-storage-service:${IMAGE_TAG}" "${SCRIPT_DIR}"
docker tag "document-storage-service:${IMAGE_TAG}" "${STORE_DOCUMENT_REPO}:${IMAGE_TAG}"

echo "Building index-documents-service image..."
docker build -f "${SCRIPT_DIR}/index-documents-service/Dockerfile" -t "index-documents-service:${IMAGE_TAG}" "${SCRIPT_DIR}"
docker tag "index-documents-service:${IMAGE_TAG}" "${INDEX_DOCUMENTS_REPO}:${IMAGE_TAG}"

echo "Building search-documents-service image..."
docker build -f "${SCRIPT_DIR}/search-documents-service/Dockerfile" -t "search-documents-service:${IMAGE_TAG}" "${SCRIPT_DIR}"
...
PARAMS=$(cat "${SCRIPT_DIR}/cloudformation/parameters/${ENVIRONMENT}.env" | tr '\n' ' ')

echo "Deploying CloudFormation stack..."
aws cloudformation deploy \
    --template-file "${SCRIPT_DIR}/cloudformation/templates/main.yaml" \
