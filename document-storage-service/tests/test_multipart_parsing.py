...
def make_multipart_body(boundary: str, content: str, content_type: str = "text/html") -> bytes:
    return (
        f"--{boundary}\r\n"
...
def make_event(boundary: str, body: bytes, base64_encode: bool = True):
    ...
    return {
        ...
        "requestContext": {
            "http": {"method": "PUT", "path": "/document", "protocol": "HTTP/1.1", "sourceIp": "1.2.3.4", "userAgent": "test"},
            "requestId": "test-123",
            "stage": "$default",
        },
...
def test_html_part_appears_in_uploaded_files(mock_aws):
    ...
    assert "document.html" in result, (
...
def test_plain_text_part_appears_in_uploaded_files(mock_aws):
    ...
    body = make_multipart_body(boundary, text_content, "text/plain")
    raw_event = make_event(boundary, body, base64_encode=True)
    event = APIGatewayProxyEventV2(raw_event)
...
def test_missing_document_part_raises_error(mock_aws):
    ...
    with pytest.raises(app_module.MultipartParsingError, match="Missing required 'document' part"):
